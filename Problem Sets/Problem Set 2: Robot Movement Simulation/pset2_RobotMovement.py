...
class RectangularRoom(object):
    ...
    def cleanTileAtPosition(self, pos):
        ...
        pos: a Position
        """
        self.cleanTiles[(math.floor(pos.getX()), math.floor(pos.getY()))] = "Clean"
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for t in range(num_trials):
        ...
        num_cleanTiles = min_coverage * room.getNumTiles()
        while room.getNumCleanedTiles() < num_cleanTiles:
            for cleaner in robots:                    # When all robots have cleaned one tile each, one time-step has passed
                # anim.update(room, robots)
                cleaner.updatePositionAndClean()
            total += 1
        # anim.done()
    return round(total / num_trials, 2)
